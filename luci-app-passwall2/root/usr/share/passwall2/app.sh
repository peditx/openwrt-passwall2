...
start_crontab() {
	...
	if [ -n "$stop_week_mode" ]; then
		...
		echolog "Configure scheduled tasks：Automatically shut down service。"
	fi
	...
	if [ -n "$restart_week_mode" ]; then
		...
		echolog "Configure scheduled tasks：Automatically restart services。"
	fi
...
start() {
	pgrep -f /tmp/etc/passwall2/bin > /dev/null 2>&1 && {
		#echolog "Program has started，Stop and then restart!"
		stop
	}
	...
	if [ "$use_nft" == 0 ]; then
		if [ -n "$(command -v iptables-legacy || command -v iptables)" ] && [ -n "$(command -v ipset)" ] && [ -n "$(dnsmasq --version | grep 'Compile time options:.* ipset')" ]; then
			...
			echolog "System is not installediptablesoripsetorDnsmasqNot turned onipsetsupport，not availableiptables+ipsettransparent proxy！"
		...
		if [ -n "$(dnsmasq --version | grep 'Compile time options:.* nftset')" ]; then
			...
			echolog "DnsmasqPackage does not satisfynftablesTransparent proxy requirements，Please make sure to usednsmasqversion in2.87above and enablenftsetsupport。"
		fi
	fi
	...
	[ "$USE_TABLES" = "nftables" ] && {
		...
		[ "$(expr $dnsmasq_version \>= 2.90)" == 0 ] && echolog "Dnsmasqversion lower than2.90，It is recommended to upgrade to2.90and above versions to avoid some situationsDnsmasqcrash problem！"
	}
