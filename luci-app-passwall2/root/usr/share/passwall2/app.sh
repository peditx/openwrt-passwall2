...
start() {
	...
	if [ "$use_nft" == 0 ]; then
		...
		if [ -n "$(dnsmasq --version | grep 'Compile time options:.* nftset')" ]; then
			...
			echolog "DnsmasqThe package is not satisfiednftablesTransparent proxy requirements，Please make sure if you need to use itdnsmasqVersion in2.87The above and turn onnftsetsupport。"
		fi
	fi
