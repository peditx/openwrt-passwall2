...
RULE_LAST_INDEX() {
	[ $# -ge 3 ] || {
		echolog "Incorrect index enumeration（nftables），Terminate execution！"
		return 1
	}
	local table_name="${1}"; shift
	local chain_name="${1}"; shift
				...
				[ -n "$shunt_node" ] && {
					...
					[ "${enable_geoview}" = "1" ] && {
						...
						[ -n "$_geoip_code" ] && {
							...
							echolog "  - [$?]Analyze the shunt rules[$shunt_id]-[geoip:${_geoip_code}]Join NFTSET Finish"
						}
					}
				}
			done
...
filter_haproxy() {
	...
	echolog "Join the load balancing node tonftset[$NFTSET_VPS]Direct connection is completed"
}
...
filter_vpsip() {
	...
	echolog "  - [$?]Join allIPv4Node tonftset[$NFTSET_VPS]Direct connection is completed"
	...
	echolog "  - [$?]Join allIPv6Node tonftset[$NFTSET_VPS6]Direct connection is completed"
}
...
add_firewall_rule() {
	...
	[ -n "$lan_ifname" ] && {
		...
		#echolog "This machineIPv4Direct network access to network segments：${lan_ip}"
		#echolog "This machineIPv6Direct network access to network segments：${lan_ip6}"
	...
	[ -n "$AUTO_DNS" ] && {
		for auto_dns in $(echo $AUTO_DNS | tr ',' ' '); do
			...
			echolog "  - [$?]Additional direct connectionDNSarrivenftables：${dns_address}:${dns_port:-53}"
		done
	}
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && {
		...
		if [ -n "$NODE" ] && [ "$TCP_LOCALHOST_PROXY" = "1" ]; then
			...
			[ -n "${is_tproxy}" ] && {
				nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol tcp iif lo $(REDIRECT $REDIR_PORT TPROXY4) comment \"This machine\""
				nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol tcp iif lo counter return comment \"This machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto tcp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"This machine\""
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto tcp iif lo counter return comment \"This machine\""
			}
		...
		if [ -n "$NODE" ] && [ "$UDP_LOCALHOST_PROXY" = "1" ]; then
			...
			nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol udp iif lo $(REDIRECT $REDIR_PORT TPROXY4) comment \"This machine\""
			nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol udp iif lo counter return comment \"This machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto udp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"This machine\""
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto udp iif lo counter return comment \"This machine\""
			}
...
del_firewall_rule() {
	...
	$DIR/app.sh echolog "delete nftables Rules completed。"
}
