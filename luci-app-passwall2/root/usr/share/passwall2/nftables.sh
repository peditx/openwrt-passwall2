				...
				[ -n "$shunt_node" ] && {
					...
					[ "${enable_geoview}" = "1" ] && {
						...
						[ -n "$_geoip_code" ] && {
							...
							echolog "  - [$?]Parse diversion rules[$shunt_id]-[geoip:${_geoip_code}]join to NFTSET Finish"
						}
					}
				}
			done
		...
		for sid in $(ls -F ${TMP_ACL_PATH} | grep '/$' | awk -F '/' '{print $1}' | grep -v 'default'); do
			...
			[ "${write_ipset_direct}" = "1" ] && {
				if [ -n "$(get_cache_var "ACL_${sid}_default")" ]; then
					...
					#diversion rulesIPlist(Import when using split node)
			...
			for i in $(cat $_acl_list); do
				...
				if ([ "$tcp_proxy_mode" != "disable" ] || [ "$udp_proxy_mode" != "disable" ]) && [ -n "$dns_redirect_port" ]; then
					[ -n "$dns_redirect_port" ] && {
						...
						[ -z "$(get_cache_var "ACL_${sid}_default")" ] && echolog "  - ${msg}Use a different node than the global configuration，AlreadyDNSForce redirect to private DNS server。"
					}
				else
...
add_firewall_rule() {
	...
	[ -n "$lan_ifname" ] && {
		...
		#echolog "local machineIPv4Network segment mutual access direct connection：${lan_ip}"
		#echolog "local machineIPv6Network segment mutual access direct connection：${lan_ip6}"
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && {
		...
		if [ -n "$NODE" ] && [ "$TCP_LOCALHOST_PROXY" = "1" ]; then
			...
			[ -n "${is_tproxy}" ] && {
				nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol tcp iif lo $(REDIRECT $REDIR_PORT TPROXY4) comment \"local machine\""
				nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol tcp iif lo counter return comment \"local machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto tcp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"local machine\""
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto tcp iif lo counter return comment \"local machine\""
			}
		...
		if [ -n "$NODE" ] && [ "$UDP_LOCALHOST_PROXY" = "1" ]; then
			...
			nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol udp iif lo $(REDIRECT $REDIR_PORT TPROXY4) comment \"local machine\""
			nft "add rule $NFTABLE_NAME PSW2_MANGLE ip protocol udp iif lo counter return comment \"local machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto udp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"local machine\""
				nft "add rule $NFTABLE_NAME PSW2_MANGLE_V6 meta l4proto udp iif lo counter return comment \"local machine\""
			}
...
del_firewall_rule() {
	...
	$DIR/app.sh echolog "delete nftables Rule completed。"
}
