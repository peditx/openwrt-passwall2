	...
	elseif szType == "ss" then
		...
		if hostInfo and #hostInfo > 0 then
			...
			if result.plugin then
				if result.type == 'Xray' then
					--No plug-in supported
					result.error_msg = "XrayNo plug-in supported."
				end
...
local function parse_link(raw, add_mode, add_from, cfgid)
	if raw and #raw > 0 then
		...
		if add_mode == "2" then
			log('Obtained【' .. add_from .. '】Subscription content is empty，It may be that the subscription address is invalid，Or network problems，Please check。')
		end
	end
end
	...
	do
		...
		if #fail_list > 0 then
			for index, value in ipairs(fail_list) do
				log(string.format('【%s】Subscription failed，It may be that the subscription address is invalid，Or network problems，Please diagnose！', value.remark))
			end
		end
		update_node(0)
	end
