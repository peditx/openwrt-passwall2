...
local function get_subscribe_info(cfgid, value)
	...
	local rem_traffic = value:match("remaining flow：(.+)")
...
local function parse_link(raw, add_mode, add_from, cfgid)
	if raw and #raw > 0 then
		...
		if add_mode == "2" then
			log('obtained【' .. add_from .. '】Subscription content is empty，It may be that the subscription address is invalid，or network problem，Please diagnose！')
		end
	end
end
