...
-- Determine whether to filter the node keyword
...
do
	...
	if true then
		...
		uci:foreach(appname, "haproxy_config", function(t)
			...
			CONFIG[#CONFIG + 1] = {
				log = true,
				id = t[".name"],
				remarks = "HAProxyList of load balancing node[" .. i .. "]",
				currentNode = node_id and uci:get_all(appname, node_id) or nil,
				set = function(o, server)
					uci:set(appname, t[".name"], option, server)
					o.newNodeId = server
	...
	uci:foreach(appname, "socks", function(o)
		...
		if node_table then
			...
			for k,node_id in ipairs(node_table) do
				if node_id then
					...
					if currentNode then
						...
						nodes[#nodes + 1] = {
							log = true,
							remarks = "Socks[" .. id .. "]List of spare nodes[" .. k .. "]",
							currentNode = currentNode,
							set = function(o, server)
								for kk, vv in pairs(CONFIG) do
									if (vv.remarks == id .. "List of spare nodes") then
			...
			CONFIG[#CONFIG + 1] = {
				remarks = id .. "List of spare nodes",
				nodes = nodes,
				new_nodes = new_nodes,
				set = function(o)
					for kk, vv in pairs(CONFIG) do
	...
	uci:foreach(appname, "nodes", function(node)
		...
		elseif node.protocol and node.protocol == '_balancing' then
			...
			if node.balancing_node then
				for k, node in pairs(node.balancing_node) do
					nodes[#nodes + 1] = {
						...
						set = function(o, server)
							for kk, vv in pairs(CONFIG) do
								if (vv.remarks == "XrayLoad balancing node[" .. node_id .. "]List") then
									table.insert(vv.new_nodes, server)
								end
							end
						end
			...
			CONFIG[#CONFIG + 1] = {
				remarks = "XrayLoad balancing node[" .. node_id .. "]List",
				nodes = nodes,
				new_nodes = new_nodes,
				set = function(o)
					for kk, vv in pairs(CONFIG) do
			...
			if currentNode and currentNode.fallback_node then
				CONFIG[#CONFIG + 1] = {
					log = true,
					id = node_id,
					remarks = "XrayLoad balancing node[" .. node_id .. "]Reserve node",
			...
			if currentNode and currentNode.preproxy_node then
				CONFIG[#CONFIG + 1] = {
					log = true,
					id = node_id,
					remarks = "node[" .. node_id .. "]Front proxy node",
...
local function processData(szType, content, add_mode, add_from)
	...
	local result = {
		timeout = 60,
		add_mode = add_mode, --0For manual configuration,1To import,2For subscription
		add_from = add_from
	}
	...
	elseif szType == 'vmess' then
		...
		if result.type == "sing-box" and (result.transport == "mkcp" or result.transport == "xhttp" or result.transport == "splithttp") then
			log("Skip node:" .. result.remarks .."，becauseSing-BoxNot support" .. szType .. "Protocol" .. result.transport .. "Transmission method，Need to be replacedXray。")
			return nil
		end
	elseif szType == "ss" then
		...
		if hostInfo and #hostInfo > 0 then
			...
			if result.plugin then
				if result.type == 'Xray' then
					--No plug -in
					result.error_msg = "XrayNo plug -in."
				end
			...
			if result.type == "SS" then
				...
				if aead2022 then
					-- shadowsocks-libev Not support2022encryption
					result.error_msg = "shadowsocks-libev Not support2022encryption."
				end
			end
	...
	elseif szType == "trojan" then
		...
		if content:find("@") then
			...
			if result.type == "sing-box" and (result.transport == "mkcp" or result.transport == "xhttp" or result.transport == "splithttp") then
				log("Skip node:" .. result.remarks .."，因Sing-BoxNot support" .. szType .. "Protocol" .. result.transport .. "Transmission method，需更换Xray。")
				return nil
			end
		end
	...
	elseif szType == "vless" then
		...
		if content:find("@") then
			...
			if result.type == "sing-box" and (result.transport == "mkcp" or result.transport == "xhttp" or result.transport == "splithttp") then
				log("Skip node:" .. result.remarks .."，becauseSing-BoxNot support" .. szType .. "Protocol" .. result.transport .. "传输方式，Need to be replacedXray。")
				return nil
			end
		end
	...
	elseif szType == 'hysteria2' or szType == 'hy2' then
		...
		if params.insecure and (params.insecure == "1" or params.insecure == "0") then
			...
			--log(result.remarks ..' 使用节点AllowInsecureset up: '.. result.tls_allowInsecure)
		else
			result.tls_allowInsecure = allowInsecure_default and "1" or "0"
		end
	...
	if config.currentNode then
					...
					if node.type and node.remarks and node.address and node.port then
						if node.type == config.currentNode.type and node.remarks == config.currentNode.remarks and (node.address .. ':' .. node.port == config.currentNode.address .. ':' .. config.currentNode.port) then
							if config.log == nil or config.log == true then
								log('renew【' .. config.remarks .. '】First match node：' .. node.remarks)
							end
							server = node[".name"]
							break
						end
				...
				if config.currentNode.type and config.currentNode.address and config.currentNode.port then
					if node.type and node.address and node.port then
						if node.type == config.currentNode.type and (node.address .. ':' .. node.port == config.currentNode.address .. ':' .. config.currentNode.port) then
							if config.log == nil or config.log == true then
								log('renew【' .. config.remarks .. '】第二匹配节点：' .. node.remarks)
							end
							server = node[".name"]
							break
						end
				...
				if config.currentNode.address and config.currentNode.port then
					if node.address and node.port then
						if node.address .. ':' .. node.port == config.currentNode.address .. ':' .. config.currentNode.port then
							if config.log == nil or config.log == true then
								log('更新【' .. config.remarks .. '】Third matching node：' .. node.remarks)
							end
							server = node[".name"]
							break
						end
				...
				if config.currentNode.address then
					if node.address then
						if node.address == config.currentNode.address then
							if config.log == nil or config.log == true then
								log('更新【' .. config.remarks .. '】Fourth matching node：' .. node.remarks)
							end
							server = node[".name"]
							break
						end
		...
		if not server then
			...
			if #nodes_table > 0 then
				if config.log == nil or config.log == true then
					log('【' .. config.remarks .. '】' .. 'Can't find the most matched node，当前已更换为：' .. nodes_table[1].remarks)
				end
...
local function update_node(manual)
	if next(nodeResult) == nil then
		log("Update failure，No node information available")
		return
	end
	...
	if manual == 0 and next(group) then
		uci:foreach(appname, "nodes", function(node)
			-- Don't delete it if you don’t find a new node or manually imported node...
	...
	for _, v in ipairs(nodeResult) do
		...
		for _, vv in ipairs(list) do
			...
			for kkk, vvv in pairs(vv) do
				...
				-- sing-box Domain name analysis strategy
			...
			if v then
				xpcall(function ()
					...
					elseif not szType then
						...
						log('Skip the unknown type: ' .. szType)
					end
					-- log(result)
					if result then
						if result.error_msg then
							log('丢弃节点: ' .. result.remarks .. ", reason:" .. result.error_msg)
						elseif not result.type then
							log('Abandon node: ' .. result.remarks .. ", 找不到可使用二进制.")
						elseif (add_mode == "2" and is_filter_keyword(result.remarks)) or not result.address or result.remarks == "NULL" or result.address == "127.0.0.1" or
							...
							log('Discard the filter node: ' .. result.type .. ' 节点, ' .. result.remarks)
						else
...
local execute = function()
	do
		...
		for index, value in ipairs(subscribe_list) do
			...
			local result = (not access_mode) and "自动" or (access_mode == "direct" and "直连访问" or (access_mode == "proxy" and "通过代理" or "automatic"))
