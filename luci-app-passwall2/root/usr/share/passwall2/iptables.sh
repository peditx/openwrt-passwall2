...
#Solve port exceeding15indivualiptinvalid，Supports single port、Port range
add_port_rules() {
	local ipt_cmd="$1"
	local port_list="$2"
	local target="$3"
...
filter_haproxy() {
	...
	echolog "Join the load balancing node toipset[$IPSET_VPS]Direct connection is completed"
}
