				...
				[ -n "$shunt_node" ] && {
					...
					[ "${enable_geoview}" = "1" ] && {
						...
						[ -n "$_geoip_code" ] && {
							...
							echolog "  - [$?]Parse diversion rules[$shunt_id]-[geoip:${_geoip_code}]join to IPSET Finish"
						}
					}
				}
			done
...
load_acl() {
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && [ "$CLIENT_PROXY" == 1 ] && {
		...
		[ "$TCP_NO_REDIR_PORTS" != "disable" ] && {
			add_port_rules "$ip6t_m -A PSW2 $(comment "default") -p tcp" $TCP_NO_REDIR_PORTS "-j RETURN"
			add_port_rules "$ipt_tmp -A PSW2 $(comment "default") -p tcp" $TCP_NO_REDIR_PORTS "-j RETURN"
			if ! has_1_65535 "$TCP_NO_REDIR_PORTS"; then
				echolog "  - ${msg}Not an agent TCP port[${TCP_NO_REDIR_PORTS}]"
			else
				TCP_PROXY_MODE="disable"
		...
		[ "$UDP_NO_REDIR_PORTS" != "disable" ] && {
			add_port_rules "$ip6t_m -A PSW2 $(comment "default") -p udp" $UDP_NO_REDIR_PORTS "-j RETURN"
			add_port_rules "$ipt_tmp -A PSW2 $(comment "default") -p udp" $UDP_NO_REDIR_PORTS "-j RETURN"
			if ! has_1_65535 "$UDP_NO_REDIR_PORTS"; then
				echolog "  - ${msg}Not an agent UDP port[${UDP_NO_REDIR_PORTS}]"
			else
				UDP_PROXY_MODE="disable"
		...
		if [ "$TCP_PROXY_MODE" != "disable" ] && [ -n "$NODE" ]; then
			...
			add_shunt_t_rule "${SHUNT_LIST4}" "$ipt_tmp -A PSW2 $(comment "default") -p tcp" "${ipt_j}" $TCP_REDIR_PORTS
			add_port_rules "$ipt_tmp -A PSW2 $(comment "default") -p tcp" $TCP_REDIR_PORTS "${ipt_j}"
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				add_shunt_t_rule "${SHUNT_LIST6}" "$ip6t_m -A PSW2 $(comment "default") -p tcp" "-j PSW2_RULE" $TCP_REDIR_PORTS
				add_port_rules "$ip6t_m -A PSW2 $(comment "default") -p tcp" $TCP_REDIR_PORTS "-j PSW2_RULE"
		...
		if [ "$UDP_PROXY_MODE" != "disable" ] && [ -n "$NODE" ]; then
			...
			add_shunt_t_rule "${SHUNT_LIST4}" "$ipt_m -A PSW2 $(comment "default") -p udp" "-j PSW2_RULE" $UDP_REDIR_PORTS
			add_port_rules "$ipt_m -A PSW2 $(comment "default") -p udp" $UDP_REDIR_PORTS "-j PSW2_RULE"
			$ipt_m -A PSW2 $(comment "default") -p udp $(REDIRECT $REDIR_PORT TPROXY)

			[ "$PROXY_IPV6" == "1" ] && {
				$ip6t_m -A PSW2 $(comment "default") -p udp -d $FAKE_IP_6 -j PSW2_RULE
				add_shunt_t_rule "${SHUNT_LIST6}" "$ip6t_m -A PSW2 $(comment "default") -p udp" "-j PSW2_RULE" $UDP_REDIR_PORTS
				add_port_rules "$ip6t_m -A PSW2 $(comment "default") -p udp" $UDP_REDIR_PORTS "-j PSW2_RULE"
...
del_firewall_rule() {
	...
	$DIR/app.sh echolog "delete iptables Rule completed。"
}
